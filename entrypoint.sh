...
# Then, pull DVC artifacts (S3 remote) at runtime and start the API server.
...
# 4) Pull DVC artifacts if missing
MODEL_PATH="models/best_gridsearch_amplio.joblib"
if [ ! -f "$MODEL_PATH" ]; then
  echo "Model not found at $MODEL_PATH. Running 'dvc pull' using profile 'equipo56'..."
